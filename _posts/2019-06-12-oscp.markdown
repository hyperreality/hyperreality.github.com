...
Due to a good set of scripts which guided me through the process, I completed the exam buffer overflow within half an hour. The scripts are based on [21y4d's work](https://github.com/21y4d/Windows_BufferOverflowx32) with additional automation and improvements lifted from Justin Steven's fantastic [dostackbufferoverflowgood tutorial](https://github.com/justinsteven/dostackbufferoverflowgood) &mdash; props to them.

You can find the scripts [here](https://github.com/hyperreality/OSCP-Buffer-Overflow-in-30-minutes).
...
RDP then becomes slightly less awkward when developing your buffer overflow exploit or performing post-exploitation.
...
That's why I feel that HackTheBox, and to a lesser extent [VulnHub](https://www.vulnhub.com/), are fantastic environments for building up that initial experience. There is so much to be gained just from working through writeups for those boxes and collecting useful commands and techniques.
